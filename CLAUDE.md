...
3. **Train Pure RL (PPO)** - Uses **DENSE rewards** and environment-optimized parameters:
   ```bash
   # Auto-optimized for NeedleReach (100k timesteps, lr=3e-4, n_steps=2048, DENSE rewards)
   docker compose -f docker/docker-compose.yml exec dvrk-dev python3 scripts/train_rl.py
   
   # Auto-optimized for PegTransfer (300k timesteps, lr=1e-4, n_steps=4096, DENSE rewards)  
   docker compose -f docker/docker-compose.yml exec dvrk-dev python3 scripts/train_rl.py --env PegTransfer-v0
   
   # Override specific parameters (automatically gets DENSE rewards)
   docker compose -f docker/docker-compose.yml exec dvrk-dev python3 scripts/train_rl.py --env PegTransfer-v0 --timesteps 200000 --learning-rate 5e-5
