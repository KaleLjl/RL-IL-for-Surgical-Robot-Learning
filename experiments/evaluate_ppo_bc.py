...
def evaluate_ppo_il_agent(env_name, model_path, n_episodes, output_dir=None, action_noise_test=False, noise_std=0.1, save_video=False, video_dir="videos", no_render=False):
    """
    ...
    """
    print(f"--- Starting PPO+IL Evaluation ---")
    print(f"Environment: {env_name}")
    print(f"Model Path: {model_path}")
    print(f"Number of Episodes: {n_episodes}")

    # --- 1. Create Environment ---
    # PPO+IL models use sparse rewards and flattened observations
    ...
    env_kwargs = {'render_mode': render_mode}  # No dense reward flag = sparse rewards
    
    env = gym.make(env_name, **env_kwargs)

    # Apply flattened wrapper (PPO+IL models were trained with this)
    ...
    if save_video:
        ...
        video_folder = os.path.join(video_dir, "ppo_il_evaluation")
        os.makedirs(video_folder, exist_ok=True)
        video_path = os.path.join(video_folder, "ppo_il_eval_combined_all_episodes.mp4")
    ...
    try:
        ...
        print("PPO+IL model loaded successfully.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a trained PPO+IL (DAPG) agent.")
    parser.add_argument(
        "--model",
        type=str,
        required=True,
        help="Path to the trained PPO+IL model (.zip file)."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print(f"Using sparse rewards (appropriate for PPO+IL)")
    print(f"Using flattened observations")

    evaluate_ppo_il_agent(
        env_name=env_name,
